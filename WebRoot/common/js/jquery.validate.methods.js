...
jQuery.validator.addMethod("isNullByTarget", function(value, element,param) {       
	var target = $(param);
	if(target.val() != null && target.val() != "" && target.val() != -1){
		if(value != null && value != ""){
			return true;
		}else{
			return false;
...
jQuery.validator.addMethod("decimal", function(value, element, param) {
	return this.optional(element) || new RegExp("^-?\\d{1," + (param.length != null ? param.length : "") + "}" + (param.scale != null ? (param.scale > 0 ? "(\\.\\d{1," + param.scale + "})?$" : "$") : "(\\.\\d+)?$")).test(value);
...
jQuery.validator.addMethod("maximum", function(value, element, param) {
	value = parseInt($(element).data("max"));
	return this.optional(element) || value >= parseInt($(element).val());
}, jQuery.validator.format("数值不正确"));

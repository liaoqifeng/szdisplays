...
<body>
	<div id="mainBox" class="main-box">
		<div id="mainPanel" class="easyui-panel" iconCls="icon-save" title="修  改  角  色">
			<form id="editForm" action="${path }/back/role/edit.shtml" method="post">
		    	...
		    	<table class="table-edit" width="100%" align="center" cellpadding="0" cellspacing="0">
		      		<tbody>
		      			<tr>
		      				...
		      				<td width="80%" align="left"><input type="text" name="name" value="${role.name }" class="text middle"/></td>
		      			</tr>
		      			<tr>
		      				<td align="right">角色标识：&nbsp;&nbsp;</td>
		      				<td align="left"><input type="text" name="value" value="${role.value }" class="text middle"/></td>
		      			</tr>
		      			<tr>
		      				<td align="right">角色描述：&nbsp;&nbsp;</td>
		      				<td align="left" height="100"><textarea rows="5" cols="40" name="description">${role.description }</textarea></td>
		      			</tr>
		      			<tr>
												...
												<td valign="middle">
													<c:if test="${row.nodes != null }">
					      								<c:forEach var="item" items="${row.nodes }" varStatus="index">
					      									<input type="checkbox" name="auths" value="${item.perms }" ${fn:containsObject(role.auths,item.perms) ? 'checked="checked"' : '' }/>${item.name }
					      								</c:forEach>
					      							</c:if>
												</td>
											</tr>

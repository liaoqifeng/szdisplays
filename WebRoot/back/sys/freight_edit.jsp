...
<body>
	<div id="mainBox" class="main-box">
		<div id="mainPanel" class="easyui-panel" iconCls="icon-add" title="&nbsp;&nbsp;修  改  运  费  模  板">
			<form id="editForm" action="${path }/back/freightTemplet/edit.shtml" method="post">
				<input type="hidden" name="id" value="${valueMap.freightTemplet.id }"/>
		    	...
		    	<table class="table-edit" align="center" cellpadding="0" cellspacing="0" width="100%">
		      		<tbody>
		      			<tr>
		      				...
		      				<td width="70%" align="left"><input type="text" name="name" value="${valueMap.freightTemplet.name }" class="text-three"/></td>
		      			</tr>
		      			<tr>
		      				<td align="right">备注：&nbsp;&nbsp;</td>
		      				<td align="left"><input type="text" name="remark" value="${valueMap.freightTemplet.remark }" class="text-three"/></td>
		      			</tr>
		      			<tr>
		      				<td align="right">排序：&nbsp;&nbsp;</td>
		      				<td align="left"><input type="text" name="orderList" value="${valueMap.freightTemplet.orderList }" field="number" class="text-num"/></td>
		      			</tr>
		      			<tr>
		      				...
		      				<c:forEach var="row" items="${valueMap.deliverways}">
		      					<tr name="deliverway" rowId="${row.id }">
				      				<td width="30%" align="right">
				      					<input type="checkbox" name="deliverway.id" value="${row.id }" ${row.id==valueMap.freightTemplet.deliverway.id?"checked='checked'":"" }/>
				      				</td>
				      				<td width="70%" align="left">${row.name }</td>
				      			</tr>
				      			<tr name="content" rowId="${row.id }" valign="top" ${row.id==valueMap.freightTemplet.deliverway.id?"":"style='display: none;'" }>
				      						...
				      						<tr>
				      							<td>
				      								<table name="config" align="center" cellpadding="0" cellspacing="0" width="100%">
							      						...
							      						<c:if test="${valueMap.freightTemplet.freightTempletAttributes != null}">
							      							<c:forEach var="item" items="${valueMap.freightTemplet.freightTempletAttributes}" varStatus="i">
							      							<tr name="${i.index }">
								      							<td>
								      								送至<input type="text" name="area" area='${item.items }' value="${item.value }" class="text-four" onkeydown="return false;" readonly="readonly"/>
								      								首重<input type="text" name="firstWeight" field="price" value="${item.firstWeight }" class="text-num"/>&nbsp;kg
								      								首重费用<input type="text" name="firstPrice" field="price" value="${item.firstPrice }" class="text-one"/>元
								      								续重<input type="text" name="conWeight" field="price" value="${item.conWeight }" class="text-num"/>&nbsp;kg
								      								续重费用<input type="text" name="conPrice" field="price" value="${item.conPrice }" class="text-num"/>元
								      								&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" name="delete">[删除]</a>
								      							</td>
								      						</tr>
							      							</c:forEach>
...
<script type="text/javascript">
	$().ready(function(){
		...
		$content.find("table[name='config'] tr:visible").each(function(){
			var $this =	$(this);
			$this.find(":text[name='area']").click(function(){
				var area = $(this).attr("area");
				var rowId = $("tr[name='deliverway']").find(":checkbox:checked").val();
				$doAddArea.attr("name",$this.attr("name"));
				initArea(area,rowId);
